...
/////////////////////////////////////////////////////
// File Name: WifiManagerWrapperForQR.kt
// File Description: This manager class for scanning
// and handling wifi for QR section.
...
import com.innoweavebiocare.matsya.LoadingCalibrationActivity

class WifiManagerWrapperForQR() {
    ...
    fun wifiManagerInti(context: Context): WifiManagerWrapperForQR {
    ...
    fun connectWifi(
        networkSSID: String, networkPassword: String, networkSecurity: String,
        ...
        if (wifiConfig == null || wifiConfig != null) {
    ...
    fun forgetWifi(
        networkSSID: String,
        wifiConnectivityCallbackResult: CalibrationActivity
    ) {
        this.connectivityListenerCallback = wifiConnectivityCallbackResult
        val wm: WifiManager =
            context.applicationContext.getSystemService(Context.WIFI_SERVICE) as WifiManager
    ...
    private fun createNetworkProfile(
        ...
        Handler().postDelayed(Runnable {
            val list = wm!!.configuredNetworks
            for (i in list) {
                if (i.SSID != null && i.SSID ==
                    String.format("\"%s\"", networkSSID)) {
