...
class LoadingCalibrationActivity : AppCompatActivity() , WifiScanCallbackResult,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.i("LoadingCalibration", "MacId $clickedDeviceID")
        ...
        Handler().postDelayed({
            try {
                wifiManagerWrapper?.connectWifi(
                    ...
                    this@LoadingCalibrationActivity
                )
                ...
                Handler().postDelayed({
                    ...
                    if (wifiManagerWrapper?.isConnectedTo(clickedDeviceID)!!) {
                        ...
                        val dialog = Dialog(this@LoadingCalibrationActivity) // Dialog
                ...
                startActivity(intent)
                finish()
                Toast.makeText(this@LoadingCalibrationActivity, "Device Unavailable, Move near the device.", Toast.LENGTH_SHORT).show()
            }        },5000)
    }
    ...
    override fun wifiFailureResult(results: MutableList<ScanResult>) {
        Log.i("LoadingCalibration", "Wi-fi Failure Result*****************= $results")
    ...
    override fun wifiSuccessResult(results: List<ScanResult>) {
        Log.i("LoadingCalibration", "Wi-Fi Success Result******************= $results")
        wifiScanResultList = emptyList()
        wifiScanResultList = results
    ...
    override fun wifiConnectionStatusChangedResult() {
        Log.i("LoadingCalibration", "************Connection Status Changed Result***********")
        checkDeviceConnected(wifiScanResultList)
    ...
    private fun checkDeviceConnected(wifiScanResultListCheck: List<ScanResult>): Boolean? {
        for (index in wifiScanResultListCheck.indices) {
            return if (wifiManagerWrapper?.isConnectedTo(wifiScanResultListCheck[index].SSID)!!) {
                ...
                Log.i("LoadingCalibration", "Connected")

                true
            } else {
                wifiScanResultList[index].capabilities = "Connection not established"
                Log.i("LoadingCalibration", "Connection not established")
